...
Calling `LatencyClocked.initialise(timers)` repeatedly is safe. The generated bind method is
invoked again and generated timer fields are rebound to timers from the latest catalogue. If
a timed method is invoked while LatencyClocked is enabled but before binding has occurred,
generated bytecode fails fast with an actionable `IllegalStateException` instead of a raw
null timer failure.
