...
When disabled, startup does not read the instrumented class-index resource or invoke generated 
bind methods; it only returns the runtime handle and logs that binding is disabled. This is an 
operational escape hatch for disabling generated timing without changing the application classpath.
